...
**Security by Design** – security is built into the core architecture of Glassbox:
- **Zero-Data Leakage**: Raw document text never leaves the backend memory boundaries. The client response contains only the redacted text and token metadata (offsets, types, confidence).
- **In-Memory Streams**: Uploaded files are processed as direct byte streams in RAM, avoiding temp file generation on disk.
- **Permanent PDF Redaction**: Rather than drawing visual black rectangles over text (which leaves underlying characters copyable and searchable), Glassbox uses PyMuPDF's (`fitz`) `add_redact_annot()` and `apply_redactions()`. Raw text segments, character coordinates, and images are physically deleted, and the document is saved with metadata scrubbing (`garbage=4, deflate=True`) to purge structural references.
- **XML-Level DOCX Redaction**: In DOCX files, PII strings are replaced at the XML run level with the literal text `"[REDACTED]"`, ensuring the original text cannot be recovered by unzipping the document package.
- **Prompt Injection Immunity**: Operating deterministically via NLP models and regex guarantees complete immunity from prompt injections, system-prompt leaks, or jailbreak attempts.
